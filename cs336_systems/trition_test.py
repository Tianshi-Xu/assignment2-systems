...
def weighted_sum_fwd(
    ...
    for i in range(tl.cdiv(D, D_TILE_SIZE)):  # pyright: ignore[reportUnreachable]
...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def forward(ctx, x, weight):
        ...
        return y.view(input_shape[:-1])  # pyright: ignore[reportUnreachable]
...
if __name__ == "__main__":
    ...
    print("Function:", weighted_sum_func)
    x = torch.randn(10,10).cuda()
    weight = torch.randn(10).cuda()
    weight.requires_grad_(True)
    x = weighted_sum_func(x, weight)
    print(x)
    # 测试反向传播
    loss = x.sum()
