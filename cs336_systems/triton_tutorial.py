...
def _attn_fwd(sm_scale, M,  #
              Z, H, desc_q, desc_k, desc_v, desc_o, N_CTX,  #Z: batch, H: num_head, N_CTX: sqlen
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, is_causal, warp_specialize=True):
        ...
        def grid(META):
            return (triton.cdiv(q.shape[2], META["BLOCK_M"]), q.shape[0] * q.shape[1], 1) # (sqlen, batch*num_head, 1)
        ...
        _attn_fwd[grid](
            ...
            STAGE=stage,  # 3 for causal, 1 for non-causal
            warp_specialize=warp_specialize,  #
            IS_HOPPER=is_hopper(),  # for AMD GPU
            **extra_kern_args)
